import random, MonstersEN as Monsters , ItemsEN as Items
map=[]
path=[]
visited=0
i=0
...
def addExits(size): # Ajoute des sorties (condition de victoire) au labyrinthe au sud and/ou a l'est
...
def getAltDescript(position): # Ajoute quelques informations a la description de base
    ...
    if len(list) > 0:
        descript += " ~~~~~~~~~~~~~~~~~~~~~~~~~ On the ground, you see "
...
def getItemList(position): # Renvoie la liste d'objets dans la zone and leur donne un index (wxcvbn) 
    descript = ""
...
def removeItem(position,index): # randire un objet a une zone
...
def removeMonster(position): # Randire le monstre de la zone
